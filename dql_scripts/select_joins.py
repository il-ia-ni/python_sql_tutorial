...
SessionTestSQLServer = sessionmaker(bind=engine_SQLServerTest_MainDB)
session = Session(engine_SQLServerTest_MainDB)

select_join_orm_stmt1 = (
    # JOIN with left and right explicit sides. See https://docs.sqlalchemy.org/en/14/tutorial/data_select.html#explicit-from-clauses-and-joins
...
select_join_orm_stmt2 = (
    # JOIN with only right-hand explicit side
    select(DefectRootCause)
    .join(DefectEvent)
    .join(SignalMeta)
